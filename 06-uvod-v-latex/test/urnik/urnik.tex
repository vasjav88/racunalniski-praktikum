...
\begin{itemize}
  ...
  \item Urnik mora zadoščati zastavljenemu časovnemu okvirju (npr.\ od sedmih
  zjutraj do osmih zvečer).
\end{itemize}
...
število šibkih omejitev, zato snovalci programov prav temu posvečajo ogromno pozornost,
...
(>odločil<) skoraj vedno, da jih ne bo prekršil. Tak način uteževanja pride v poštev
...
\noindent Z zanko se sprehodimo po vektorju vseh predmetov. Predmet na \(i\)-tem koraku
označimo s \( \text{predmet}_i \).

\noindent Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na
\(j\)-tem koraku označimo s \( \text{predavalnica}_j \).

\noindent Za vse proste termine \( \text{predavalnica}_j \) izračunamo ceno vstavljanja
\( \text{srečanje}_i \), ki ga sestavlja \( \text{predmet}_i \), profesor, ki ta predmet predava,
ter skupina, kateri je predmet namenjen.

\noindent Če je trenutna cena boljša od najboljše dosedanje, potem najboljša dosedanja
dobi vrednost trenutne. Hkrati si zapomnimo, za kateri termin smo izračunali ceno
vstavljanja, da bomo \( \text{srečanje}_i \) na koncu vstavili v termin z najboljšo
ceno vstavljanja.

\noindent Ko preverimo vse proste termine vseh predavalnic, vstavimo \( \text{srečanje}_i \)
v termin z najboljšo ceno.

\noindent Sedaj se ponovno vrnemo na začetek in pogledamo najugodnejši termin za naslednji
...
seznam prepovedanih sprememb. Podobno naredimo za vse spremembe v nadaljnjih korakih. S
tem poskrbimo, da se lahko v isti lokalni maksimum vrnemo šele čez \(n\) korakov, saj
...
\subsubsection{Končni algoritem}
...
\noindent V prvem koraku izvedemo požrešno metodo, ki rešitev urnika pripelje v lokalni maksimum.

\noindent Zanko izvajamo, dokler ne dosežemo enega izmed ustavitvenih pogojev.

\noindent Rešitev \(r\), ki smo jo dobili s pomočjo požrešne metode, izboljšamo z lokalnim vzpenjanjem
...
\noindent Na vsakem koraku ocenimo izboljšano rešitev \(r\). Če je vrednost kriterijske funkcije
